...
if [ -n "$SUPABASE_URL" ]; then
  echo "[start] Supabase mode — lexical + dense served from $SUPABASE_URL"
elif [ ! -f "data/qdrant_remote/chunks.jsonl" ] && [ -n "$DATA_URL" ]; then
  ...
  echo "[start] WARNING: data/qdrant_remote/chunks.jsonl not found and DATA_URL/SUPABASE_URL not set."
  echo "         The API will start with 0 chunks (health will show 0)."
  echo "         Push data via Git LFS or set DATA_URL/SUPABASE_URL (see DEPLOY_RENDER.md)."
fi
